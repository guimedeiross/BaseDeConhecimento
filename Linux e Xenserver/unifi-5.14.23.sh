...
if [[ "${os_codename}" =~ (disco|eoan|focal) && "${architecture}" =~ (amd64|arm64) ]]; then
  ...
  if [[ "${architecture}" == "amd64" ]]; then
    ...
    if wget "${wget_progress[@]}" -qO "$libssl_temp" 'http://security.ubuntu.com/ubuntu/pool/main/o/openssl1.0/libssl1.0.0_1.0.2n-1ubuntu5.3_amd64.deb' &>> "${eus_dir}/logs/libssl.log"; then echo -e "${GREEN}#${RESET} Successfully downloaded libssl!"; else echo -e "${RED}#${RESET} Failed to download libssl..."; abort; fi
  fi
...
if ! dpkg -l | grep "^ii\\|^hi" | grep -iq "mongodb-server\\|mongodb-org-server"; then
  ...
  if [[ "${os_codename}" =~ (precise|maya|trusty|qiana|rebecca|rafaela|rosa|xenial|sarah|serena|sonya|sylvia) && ! "${architecture}" =~ (amd64|arm64) ]]; then
    ...
    if wget "${wget_progress[@]}" -qO "$mongo_tools_temp" 'http://ftp.nl.debian.org/debian/pool/main/m/mongo-tools/mongo-tools_3.2.11-1+b2_i386.deb' &>> "${eus_dir}/logs/mongodb_install.log"; then echo -e "${GREEN}#${RESET} Successfully downloaded mongo-tools 3.2.11!"; else echo -e "${RED}#${RESET} Failed to download mongo-tools 3.2.11..."; abort; fi
    echo -e "\\n${WHITE_R}#${RESET} Installing mongo-tools 3.2.11..."
    ...
    if wget "${wget_progress[@]}" -qO "$mongodb_clients_temp" 'http://ftp.nl.debian.org/debian/pool/main/m/mongodb/mongodb-clients_3.2.11-2+deb9u1_i386.deb' &>> "${eus_dir}/logs/mongodb_install.log"; then echo -e "${GREEN}#${RESET} Successfully downloaded mongodb-clients 3.2.11!"; else echo -e "${RED}#${RESET} Failed to download mongodb-clients 3.2.11..."; abort; fi
    echo -e "\\n${WHITE_R}#${RESET} Installing mongodb-clients 3.2.11..."
    ...
    if wget "${wget_progress[@]}" -qO "$mongodb_server_temp" 'http://ftp.nl.debian.org/debian/pool/main/m/mongodb/mongodb-server_3.2.11-2+deb9u1_i386.deb' &>> "${eus_dir}/logs/mongodb_install.log"; then echo -e "${GREEN}#${RESET} Successfully downloaded mongodb-server 3.2.11!"; else echo -e "${RED}#${RESET} Failed to download mongodb-server 3.2.11..."; abort; fi
    echo -e "\\n${WHITE_R}#${RESET} Installing mongodb-server 3.2.11..."
